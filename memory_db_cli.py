...
class DatabasePathResolver:
    ...
    def resolve_database_path(self) -> Optional[Path]:
        ...
        try:
            ...
            config_path = (
                data_dir / "config" / "astrbot_plugin_angel_memory_config.json"
            )
            provider_id = ""
    ...
    def hybrid_search(
        ...
        with console.status("[bold green]执行混合检索中...") as status:
            ...
            vector_results = self._vector_search(
                collection_name, query, limit=limit * 2
            )
            ...
            try:
                ...
                if top_30_ids:
                    retrieved_items = collection.get(
                        ids=top_30_ids, include=["metadatas", "documents"]
                    )
                    export_data = []
                    ...
                    console.print(
                        f"✅ [green]已将前 30 个向量搜索结果保存到 {export_path}[/green]"
                    )
            except Exception as e:
            ...
            vector_results = self._vector_search(
                collection_name, query, limit=RECALL_COUNT
            )
            vector_scores = {res["id"]: res["similarity"] for res in vector_results}
            ...
            filtered_by_score = [
                res for res in vector_results if res["similarity"] >= VECTOR_THRESHOLD
            ]

            if not filtered_by_score:
                console.print(
                    f"[yellow]⚠️  没有向量分数高于 {VECTOR_THRESHOLD} 的文档，跳过 BM25 精排。[/yellow]"
                )
                bm25_results = []
            else:
                # 3. 数量限制
                ...
                console.print(
                    f"[green]✅ 向量召回 {len(vector_results)} 条，过滤后剩余 {len(filtered_by_score)} 条，取前 {len(candidates_for_rerank)} 条进行 BM25 精排。[/green]"
                )

                # 4. 准备精排数据
                candidate_ids = [c["id"] for c in candidates_for_rerank]
                retrieved_items = collection.get(
                    ids=candidate_ids, include=["metadatas"]
                )
                id_to_content = {
                    item["id"]: item["metadata"].get("content", "")
                    for item in zip(
                        retrieved_items["ids"], retrieved_items["metadatas"]
                    )
                }
            ...
            max_bm25_score = (
                max(score for _, score in bm25_results) if bm25_results else 1.0
            )
            bm25_scores = {
                doc_id: score / max_bm25_score for doc_id, score in bm25_results
            }
            ...
            for doc_id in all_ids:
                ...
                content = (
                    items_data.get(doc_id, {}).get("metadata", {}).get("content")
                    or items_data.get(doc_id, {}).get("document")
                    or "N/A"
                )
...
def main():
    ...
    console.print(
        Panel(
            "[bold cyan]欢迎使用 Angel Memory 交互式调试工具[/bold cyan]", expand=False
        )
    )
    ...
    mode = Prompt.ask(
        "请选择集合 (memory, note)", choices=["memory", "note"], default="note"
    )
